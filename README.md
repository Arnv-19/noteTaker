...
- **Built-in Web Browser (🌐):** Open a browser tab right in the app (reuses the Chromium engine already bundled — no extra dependency). Type a URL or a Google search, then **📸 Clip** the page — the screenshot is saved and appended to a `Web Clippings.md` note in your folder, so research goes straight into your notes. Links that open a new tab (e.g. YouTube) stay in-app as their own closable tab. Close any tab with its **✖ Close** button or `Ctrl+W`.
- **One-Step Web Capture:** Select text on a web page and hit **📋 To Note** (or `Ctrl+N`) — it's appended to `Quick Notes.md` as a quote with the source link and opened in the editor, no manual copy-paste. With nothing selected, the same command just opens Quick Notes so you can type. Heading keys (`Alt+1`…) work on captured text immediately.
