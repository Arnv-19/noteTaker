...
- **Import PDF Annotations:** Rebuild a PDF's annotation tree from a previously exported `.md` (File ▸ Import PDF Annotations from .md).
- **Workspaces:** On first launch you pick a workspace — **📚 Annotator + Notes** (PDFs, notes, media — lightweight) or **🌐 Study** (adds the in-app browser and YouTube player). Switch anytime from **☰ File ▸ 🧭 Workspace**; the web tools show/hide live and your choice is remembered.
- **Markdown Editor (Obsidian-style):** Create and edit `.md` notes with a live preview. Open a note in **reading (Preview)** mode, **double-click** or `Ctrl+E` to edit the raw markdown. Embed images **and video** exactly like Obsidian — drop a file in via 🖼/🎬 (copied into `attachments/` and linked as `![[...]]`), rendered inline with video playing in the note. The preview matches your active theme. (This one editor is the single reading/editing surface — there's no separate viewer to learn.)
- **Notes Sidebar (no vault required):** Toggle 🗂 to show a collapsible sidebar. Click 📂 inside it to browse **any folder** — your Obsidian vault, a downloads folder, a lectures folder — no vault setup needed. It lists `.md` notes **and media** (🎬 video, 🎵 audio, 🖼 images). Click a lecture video to play it right there. Right-click a file for **🗑 Delete**. Remembered between sessions.
- **Notes Open as Center Tabs:** Clicking a note opens it **in the middle area as a tab** — just like PDFs. Open as many notes side-by-side as you want, each in its own closable tab. Obsidian-style reading flow: a note opens in **preview (reading) mode**; **double-click the preview** (or press `Ctrl+E`) to drop into the raw-markdown editor — you see the `#` headings and `**bold**` source, edit it, then `Ctrl+E` back to reading. (A side-by-side **Split** view is also available in the mode dropdown.) Click a PDF tab to jump back to your PDF — open notes stay loaded in their tabs.
- **Live Note Outline:** The **Outline** panel doubles as a live table of contents for the active note — every `#`–`######` heading appears as a nested tree that updates as you type. Click a heading to jump straight to it in the editor (just like a PDF's TOC).
- **Study Browser (🌐, Study workspace):** Open a browser tab for reading web pages (reuses the bundled Chromium — no extra dependency). Type a URL or a Google search. It runs on a **persistent profile with a normal Chrome user-agent**, so logins stick and you get far fewer CAPTCHAs than a default Qt browser. **⊞ Split** opens a **second web pane** beside the first — handy for keeping an AI chat open next to what you're reading. Links that open a new tab stay in-app as their own closable tab. Close with **✖ Close** or `Ctrl+W`.
- **Focus Filter (🛡, View menu):** An optional in-app blocklist that blocks distracting/adult domains while you study. *Note:* this is a light in-app filter — for real, device-wide filtering set **Cloudflare Family DNS** (`1.1.1.3` / `1.0.0.3`) at your OS or router, which also covers this browser.
- **Side-by-Side Web Notes:** The study browser has a **note pane right next to the page** (📝 Note pane) so you see both at once. It's a **real `.md` file you choose** — 📂 open an existing note to continue it, or save a new one under any name. Select text on the page and hit **📋 To Note** (or `Ctrl+N`) — it lands in the note as a sourced quote (link added once per page) and **auto-saves**. **📸 Clip** drops a page screenshot in the same way. Heading keys (`Alt+1`…), bold, quote, and list all work inside the pane.
- **YouTube Player (🎬, Study workspace):** Paste a **video or playlist** link and *only* the embedded player renders — no distracting youtube.com chrome. A **⏱ Grab time → note** button drops the current play position into your note as a clickable, seekable timestamp (`[12:34]`), so click it later to jump the video back to that moment — ideal for lecture notes. Same side-by-side note pane as the study browser.
- **Media Playback + System-Player Fallback:** Video/audio open in an inline player; formats Qt can't decode (e.g. H.264/MP4, common for lecture recordings) get a one-click **▶ Open in system player** button that hands off to VLC/Media Player — reliable for every codec, with zero extra CPU in-app.
...
2. **Click a note** to open it as a tab in the middle area (reading view first — **double-click to edit**, like Obsidian). Click a **🎬 video / 🎵 audio / 🖼 image** to open it in the built-in player — handy for downloaded lectures. **Right-click** a file in the sidebar to delete it. Or use **📝 New Markdown Note** / **✏ Edit Markdown Note** from the File menu.
