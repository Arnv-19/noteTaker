...
- **Built-in Web Browser (🌐):** Open a browser tab right in the app (reuses the Chromium engine already bundled — no extra dependency). Type a URL or a Google search. Links that open a new tab (e.g. YouTube) stay in-app as their own closable tab. Close any tab with its **✖ Close** button or `Ctrl+W`.
- **Side-by-Side Web Notes:** The browser has a **note pane right next to the page** (📝 Note pane) so you see both at once. It's a **real `.md` file you choose** — 📂 open an existing note to continue it, or save a new one under any name. Select text on the page and hit **📋 To Note** (or `Ctrl+N`) — it lands in the side note as a quote with the source link, and **auto-saves** to the file. **📸 Clip** drops a page screenshot in the same way. Heading keys (`Alt+1`…), bold, quote, and list all work inside the pane.
